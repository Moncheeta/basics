...
class Tic_Tac_Toe:
    def __init__(self):
        ...
        self.positions = [
            [[None for a in range(3)] for b in range(self.rows)]
            for c in range(self.cols)
        ]  # keeps track of what each player placed
    ...
    def win_check(self):  # checks for win
        ...
        for row in self.positions:
            if (
                self.positions[row_num][0][0] is None
                or self.positions[row_num][1][0] is None
                or self.positions[row_num][2][0] is None
            ):
                pass
            else:
        ...
        while col != self.cols:
            if (
                self.positions[0][col][0] is None
                or self.positions[1][col][0] is None
                or self.positions[2][col][0] is None
            ):
                pass
            else:
                if (
                    self.positions[0][col][0]
                    == self.positions[1][col][0]
                    == self.positions[2][col][0]
                ):  # checks for any filled coleums
            ...
            col += 1
        if (
            self.positions[0][0][0] is None
            or self.positions[1][1][0] is None
            or self.positions[2][2][0] is None
        ):
            pass
        else:
            if (
                ...
                return True
        if (
            self.positions[0][2][0] is None
            or self.positions[1][1][0] is None
            or self.positions[2][0][0] is None
        ):
            pass
        else:
            if (
        ...
        for (
            row
        ) in self.positions:  # checks for filled game. If it is filled, no = False
            for thing in row:
                if thing[0] is None:
                    no = True
        if no is False:
